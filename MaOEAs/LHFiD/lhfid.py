...
class HFIDEAD(GeneticAlgorithm):
    ...
    def _initialize(self):
        ...
        super()._initialize()
        self.ideal_point = np.min(np.array([ind.F for ind in self.pop if ind.CV==0]), axis=0)
    ...
    def _next(self):
        ...
        temp = np.min(np.array([ind.F for ind in self.off if ind.CV==0]), axis=0)
        self.ideal_point = np.min([temp, self.ideal_point], axis=0)
        Q, CV = self.pop.get("F"), self.pop.get("CV")
        Q = np.array([Q[i] for i in range(self.pop_size) if CV[i]==0]) # Q has only feasible solutions
        ...
        # update the termination archives
        update_termination_archives(self,Q)
...
def check_for_termination(self,n_p,ns):
    ...
    to_terminate = False
    if self.n_gen>ns:# and max(self.min_constr_value[-ns:])==0:
        D_t = self.D_t[-ns:]
        S_t = self.S_t[-ns:]
        D_t = [round(val,n_p) for val in D_t]
        S_t = [round(val,n_p) for val in S_t]
