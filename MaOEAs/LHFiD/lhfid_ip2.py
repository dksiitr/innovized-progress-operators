...
def survival_selection(self):
    ...
    # feas_pop[ind1].set('rank', 0) - put this

    # divide the dominated and non-dominated sets in the feasible pop
    dom_i, non_dom_i = [],[] # the indices of dom and non-dom pop in feasible pop
    for i in range(len(F_norm)):
        counting = 0
        for j in range(len(F_norm)):
            if sum(F_norm[j]<F_norm[i])>0 and sum(F_norm[j]>F_norm[i])==0:
                counting += 1
                break
        if counting == 0:
            non_dom_i.append(i)
            feas_pop[i].set('rank', 0)
        else:
            dom_i.append(i)
    ...
    # Selection of non-dominated solutions - for non-empty clusters
    for i in range (len(self.ref_dirs)):
        cluster_indices = [p for p in non_dom_i if assoc_index[p]==i]
        cluster_perp_values = [assoc_value[p] for p in non_dom_i if assoc_index[p]==i]
    ...
    # filling of leftover non-dominated solutions
    left_solutions = [i for i in non_dom_i if i not in survivors]
    left_vectors = [i for i in range(len(self.ref_dirs)) if i not in selected_vectors]
    if min(len(left_solutions),len(left_vectors))>0:
        if len(left_solutions)<=len(left_vectors):
            for index in left_solutions:
                dist = dist_matrix[index,left_vectors]
                vector_index = np.argmin(dist)
                survivors.append(index)
                selected_vectors.append(left_vectors[vector_index])
                # update the left vectors
                left_vectors = [i for i in range(len(self.ref_dirs)) if i not in selected_vectors]
        else:
            for vector_index in left_vectors:
                dist = dist_matrix[left_solutions,vector_index]
                index = np.argmin(dist)
                survivors.append(left_solutions[index])
                selected_vectors.append(vector_index)
                # update the leftover solutions
                left_solutions = [i for i in non_dom_i if i not in survivors]

    # Selection of dominated solutions - for non-empty clusters
    left_vectors = [i for i in range(len(self.ref_dirs)) if i not in selected_vectors]
    if len(left_vectors)>0:
        for i in left_vectors:
            cluster_indices = [p for p in dom_i if assoc_index[p]==i]
            cluster_perp_values = [assoc_value[p] for p in dom_i if assoc_index[p]==i]
            if len(cluster_indices)==1:
                survivors.append(cluster_indices[0])
                selected_vectors.append(i)
            elif len(cluster_indices)>1:
    ...
    # filling of leftover dominated solutions
    left_solutions = [i for i in dom_i if i not in survivors]
    left_vectors = [i for i in range(len(self.ref_dirs)) if i not in selected_vectors]
    if min(len(left_solutions),len(left_vectors))>0:
        for vector_index in left_vectors:
            dist = dist_matrix[left_solutions,vector_index]
