...
class GeneticAlgorithm(Algorithm):
    ...
    def _next(self):
        ...
        if self.do_trigger1==False and self.is_do_learn:
            if self.problem.n_constr==0:
                ...
                CV = self.pop.get("CV")
                if max(CV)==0:
                    # non_dom = [ind.data["rank"] for ind in self.pop if ind.data['rank']==0]
                    # if len(non_dom)>=self.pop_size:
                    self.do_trigger1 = True
        ...
        # df1 = self.pop.get("F")[int(self.pop_size/2):]
        ...
        if self.is_do_learn and self.do_trigger2 and self.n_gen-self.last_do_learn==0:
            temp = [1 for model in self.do_models if model is None]
            if sum(temp)==0:
                if self.do_survived[-1]>self.do_survived[-2] and self.do_frequency>1:
                    self.do_frequency-=1
                elif self.do_survived[-1]<self.do_survived[-2]:
                    self.do_frequency+=1
        
        """Check for starting DL operator"""
        if self.is_do_learn and self.do_trigger1==True and self.do_trigger2==False:
    ...
    def co_learn(self):
        ...
        obj_data = normalize(obj_data, x_min = ideal_point, x_max = nadir_point)
        ...
        regr = RandomForestRegressor(max_features=self.problem.n_var, min_samples_split=2, random_state=1, n_estimators=int(self.pop_size))
        regr.fit(input_data,output_data)

        return lower_bound,upper_bound, regr

    def co_boundary_repair(p,c,l,u,iters):
        # low, high = deepcopy(l), deepcopy(u)
        # l = np.zeros(len(l))
        # u = np.ones(len(u))
        # p = normalize(p,x_min=low,x_max=high)
        # c = normalize(c,x_min=low,x_max=high)
        p[p==c] = (u+l)[p==c]/2 #to make the equal values differ and the function returns a NaN value
        ...
        if sum(idl+idr)==0:
            ...
            if r==0:
                ...
                Y = d*(1+alpha*atan)
                data = c + (p-c)*Y/normv
        # data = denormalize(data,x_min=low,x_max=high)
        # p = denormalize(p,x_min=low,x_max=high)
        # c = denormalize(c,x_min=low,x_max=high)
        ...
        if sum(idl+idr)>0:
            ...
            if iters<10:
                ...
                #raise NameError('Invalid Boundary Handling')
    ...
    def co_repair(self, lower_bound, upper_bound, regr):
        ...
        for i in range (0,int(self.pop_size*self.co_repair_fraction)):
            ...
            self.co_boost = 0.5 + np.random.random()
    ...
    def update_co_target(self):
        ...
        if max(nadir_point) == math.inf:
            nadir_point = np.max(self.pop.get("F"), axis=0)
        F_norm = normalize(F, x_min = ideal_point, x_max = nadir_point)
        assoc_index = []
        assoc_value = []
        for obj in F_norm:
        ...
        for i in range(self.pop_size):
            ...
            if self.co_target[index][0][0] is None:
                ...
                target_F = normalize(self.co_target[index][1], x_min = ideal_point, x_max = nadir_point)
        ...
        #     temp = []
        #     index = []
        #     for j in range(len(assoc_index)):
        #         if assoc_index[j]==i:
        #             temp.append(assoc_value[j])
        #             index.append(j)
        #     if index!=[]:
        #         j = np.argmin([np.linalg.norm(F_norm[z]) for z in index])
        #         if self.co_target[i][0][0] is not None:
        #             old = normalize([self.co_target[i][1]], x_min = ideal_point, x_max = nadir_point)
        #             old = old[0]
        #             new = F_norm[index[j]][:]
        #             old = np.linalg.norm(old)
        #             new = np.linalg.norm(new)
        #             if new<old:
        #                 self.co_target[i][0] = X[index[j]][:]
        #                 self.co_target[i][1] = F[index[j]][:]
        #         else:
        #             self.co_target[i][0] = X[index[j]][:]
        #             self.co_target[i][1] = F[index[j]][:]
    ...
    def update_termination_archives(self,Q):
        ...
        P_norm = normalize(P, x_min=ideal_point, x_max=nadir_point)
        Q_norm = normalize(Q, x_min=ideal_point, x_max=nadir_point)
