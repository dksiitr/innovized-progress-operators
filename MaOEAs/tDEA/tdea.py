...
class ReferenceDirectionSurvival(Survival):
    ...
    def _do(self, problem, pop, n_survive, D=None, **kwargs):
        ...
        for i in range(len(F)):
            if max(self.ref_dirs[niche_of_individuals[i]])==1:
                decomposition = get_decomposition("pbi", theta=1000000)
        ...
        if len(pop) > n_survive:
            ...
            while len(survivors) < n_survive:
                select = [row[trank] for row in clusters if len(row)>=trank+1]
                select = np.random.permutation(select)
                for val in select:
...
parse_doc_string(TDEA.__init__)
